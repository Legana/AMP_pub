...
ampir_prec_bench_raw <- predict_bench("cache/ampir_prec_bench_raw.rds",ampir_prec_bench )
...
ampir_prec_bench_ampep <- read_csv("raw_data/benchmarking/results/ampep/precursor_eval_ampep.txt") %>% 
  mutate(actual = ifelse(grepl(Row,pattern = "_Tg$"), "Tg", "Bg")) %>% 
  rename(prob_AMP = score) %>% 
  mutate(predicted = ifelse(prediction == 1, "Tg", "Bg")) %>%
  mutate(actual = factor(actual), predicted = factor(predicted))
...
ampir_bench_ampscanv2 <- read_csv("raw_data/benchmarking/results/ampscanv2/ampir/1588756293516_Prediction_Summary.csv") %>% 
  mutate(actual = ifelse(grepl(SeqID,pattern = "^AP"), "Tg", "Bg")) %>% 
  rename(prob_AMP = Prediction_Probability) %>%
  mutate(predicted = ifelse(Prediction_Class == "AMP", "Tg", "Bg")) %>%
  mutate(actual = factor(actual), predicted = factor(predicted))

ampir_prec_bench_ampscanv2 <- read_csv("raw_data/benchmarking/results/ampscanv2/ampir_prec/1588756490979_Prediction_Summary.csv") %>% 
  mutate(actual = ifelse(grepl(SeqID,pattern = "_Tg$"), "Tg", "Bg")) %>% 
  rename(prob_AMP = Prediction_Probability) %>%
  mutate(predicted = ifelse(Prediction_Class == "AMP", "Tg", "Bg")) %>%
  mutate(actual = factor(actual), predicted = factor(predicted))
...
ampir_prec_bench_iampred <- read_tsv("raw_data/benchmarking/results/iamppred/ampir_precursor.csv") %>%
   mutate(actual = ifelse(grepl(name_fasta,pattern = "_Tg$"), "Tg", "Bg")) %>%
   mutate(prob_AMP = pmax(antibacterial,antiviral,antifungal)) %>%
   mutate(predicted = ifelse(prob_AMP >= 0.5, "Tg", "Bg")) %>%
   mutate(actual = factor(actual), predicted = factor(predicted))
...
```{r}
models_roc_mature <- rbind(ampir_roc,ampir_bench_roc, 
...
pp <- ggplot(models_roc_mature %>% filter(benchmark=="ampir")) + 
...
ampir_prec_bench_data <- list(ampir_mature = ampir_prec_bench %>% filter(model=="ampir_mature"),
                         ...
                         iamppred = ampir_prec_bench_iampred)


ampir_prec_bench_stats <- sapply(ampir_prec_bench_data,get_performance_stats)
```
...
*ampir*: ampir was run using the same Intel Xeon processor as was used for the ampep benchmark. Since ampir is capable of multicore operation we measured its runtime as a function of core-count (see figure 5.4).  In Table 5.5 the runtime of ampir with a single core is shown.  ampir provides comparable performance to ampscan v2 when run with 4 cores.
...
ggplot(ampir_human_bench_runtimes,aes(x=cores,y=time)) + geom_line() + geom_point() + xlab("Number of cores") + ylab("Elapsed time (s)")
```
**Figure 5.4** Performance of ampir as a function of core count when running `predict_amps()` on a dataset of 77000 proteins. 
