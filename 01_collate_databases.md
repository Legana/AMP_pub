...
-   [APD 3](https://wangapd3.com/main.php) by [Wang et
...
After unpacking they should be present at the following file locations:
...
The [Antimicrobial Peptide Database](https://wangapd3.com/main.php)
appears to be regularly updated. An AMP sequence FASTA file from
September 2020 is downloadable from the [APD download
page](https://wangapd3.com/downloads.php) which currently contains 3,167
sequences. To include a more updated AMP (October 2020) list, the [web
query interface](https://wangapd3.com/database/query_input.php) was used
to obtain the full 3,257 AMPs along with sufficient metadata to filter
unwanted entries.

### DRAMP

[DRAMP’s download page](http://dramp.cpu-bioinfor.org/downloads/)
provides access to download a general AMP dataset (which contains both
natural and synthetic AMPs). However, there is also a [natural
dataset](http://dramp.cpu-bioinfor.org/browse/NaturalData.php). Because
the natural dataset contains the AMPs we are interested in and is likely
more regularly updated (it currently contains 4414 sequences), the
natural AMP data was obtained using a [scrape
script](scripts/scrape_dramp.sh).
...
**note: Unable to access dbAMP database on 7 April 2021 (website
unavailable) and was therefore unable an updated version of this
database**

### UniProt

AMPs were downloaded from UniProt on 09-April-2021 using the search
term: “keyword:Antimicrobial \[KW-0929\]”. This included 3350 reviewed
and 42745 unreviewed proteins.
...
![](01_collate_databases_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->
...
![](01_collate_databases_files/figure-gfm/unnamed-chunk-11-1.png)<!-- -->

Another indicator that a protein is an AMP precursor is the presence of
a signal peptide. 767 of the 884 precursors identified above have well
...
(i.e. reflecting other types of mature peptides).

![](01_collate_databases_files/figure-gfm/unnamed-chunk-16-1.png)<!-- -->
...
This leaves an initial database with 2294 entries, of which 213 are
unreviewed.
