...
As a final step we write the database to a FASTA file and then use `cd-hit` to cluster sequences to 90% identity, keeping only a single representative sequence for each cluster.  This reduces the database size but roughly maintains the same length distribution.
...
ggplot(ampir_db_summary,aes(x=Length)) + geom_histogram() + facet_wrap(~in_90,ncol = 1)
```
...
ampir_db_summary %>% group_by(Organism) %>% 
  summarise(nentries = n(), n90_entries = sum(in_90)) %>% arrange(desc(n90_entries)) %>% head()
```
...
1. Include all AMPs from the APD, DRAMP and dbAMP databases with lengths >20AA and < 60AA
...
ampir_mature <- combined %>% 
  ...
  filter(length < 60) %>% filter(length > 20) %>% 
  filter(database!="SwissProt" | is_mature_peptide) %>% 
  distinct(seq, .keep_all = TRUE)   %>%                  # Filtering criterion 5
  filter(grepl(seq,pattern='^[ARNDCEQGHILKMFPSTWYV]+$')) # Filtering criterion 6

ggplot(ampir_mature,aes(x=length)) + geom_histogram() + facet_wrap(~database)
```
